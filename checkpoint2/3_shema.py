...
class Room(Base): 
    __tablename__ = 'rooms'
    id = Column(Integer, primary_key=True, autoincrement=True)
    reserved_room_type = Column(String(10), nullable=False, unique=True)
...
countries_unique = df[['country']].drop_duplicates().rename(columns={'country': 'name'})
countries_list = []
for idx, (_, row) in enumerate(countries_unique.iterrows(), 1):
    try:
        res = requests.get(f"https://restcountries.com/v3.1/name/{row['name']}?fullText=true", timeout=5)
        data = res.json()
        pop = data[0].get('population', 0) if isinstance(data, list) else 0
        reg = data[0].get('region', 'Unknown') if isinstance(data, list) else 'Unknown'
    except:
        pop, reg = 0, 'Unknown'
    countries_list.append({"id": idx, "name": row['name'], "population": pop, "region": reg})
...
rooms_data = df[['reserved_room_type']].drop_duplicates()
session.execute(insert(Room), rooms_data.to_dict(orient="records"))

customers_data = df[['customer_type']].drop_duplicates()
...
room_map = {r.reserved_room_type: r.id for r in session.query(Room).all()}
...
df['room_fk'] = df['reserved_room_type'].map(room_map)
...
final_bookings = df[[
    ...
    'adults', 'children', 'adr', 
    'country_fk', 'hotel_fk', 'meal_fk', 'room_fk', 'customer_fk'
]]
