...
class TestHotelDatabase(unittest.TestCase):
    def setUp(self):
        ...
        self.df = self.df.dropna(subset=['country', 'hotel', 'meal'])
        self.df['children'] = self.df['children'].fillna(0).astype(int)
    ...
    def test_dataframes(self):
        df_sorted = self.df.sort_values(['lead_time', 'adr', 'country', 'hotel']).reset_index(drop=True)
        db_df_sorted = self.db_df.sort_values(['lead_time', 'adr', 'country', 'hotel']).reset_index(drop=True)
